...
import { Check, Clock, ShieldCheck, MapPin, Truck, BadgeCheck, Star } from 'lucide-react';
...
export default function TrackingPage() {
  ...
  useEffect(() => {
    if (activeOrder?.isPaid && !activeOrder?.hasReview) {
      const timer = setTimeout(() => setReviewOpen(true), 800);
      return () => clearTimeout(timer);
    }
  }, [activeOrder?.isPaid, activeOrder?.hasReview]);
  ...
  return (
    <div className="bg-[#0B0B0C] text-white min-h-screen font-sans flex items-center justify-center p-6 select-none">
      <div className="w-full max-w-xl bg-[#0C0C0E] border border-white/5 rounded-3xl shadow-2xl overflow-hidden">
        {/* Gold accent strip */}
        ...
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <div>
            <h3 className="font-serif text-xl font-bold text-white">Gourmet Order Tracker</h3>
            {activeOrder && (
        ...
        <div className="p-6">
          {!activeOrder ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <Truck className="w-12 h-12 text-white/15 mb-4" />
              <h4 className="text-white font-serif font-bold text-base mb-1">No Active Orders</h4>
              <p className="text-xs text-white/40 max-w-xs leading-relaxed">
                You do not have any premium gourmet orders in progress on this device. Place an order to begin tracking in real-time!
              </p>
            </div>
          ) : (
